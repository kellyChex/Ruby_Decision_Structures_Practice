...
nineLines = snowy_lines.each_slice(9).to_a
nineLines.map do |stanza|
  [0,4].each do |n|
    puts "#{last_word stanza[0+n]}, #{last_word stanza[2+n]}"
    puts
    puts "#{last_word stanza[1+n]}, #{last_word stanza[3+n]}"
    puts
  end
end
